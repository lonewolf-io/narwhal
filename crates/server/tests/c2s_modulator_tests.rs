...
#[monoio::test(enable_timer = true)]
async fn test_c2s_channel_persist_not_allowed_for_authenticated_users_when_disabled() -> anyhow::Result<()> {
  let modulator = TestModulator::new()
...
async fn test_c2s_channel_persist_allowed_for_authenticated_users() -> anyhow::Result<()> {
  ...
  let mut suite = C2sSuite::with_modulator(config, Some(s2m_client), None).await?;
  suite.setup().await?;
